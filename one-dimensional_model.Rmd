...
As we have seen, unidimensional model applied to this set of 8 questions doesn't fit data very well. We can look for some more complicated model specifications that will allow better fit.

Within CCFA approach we have two solutions that can be used to improve model fit:

  - Assuming that there are **correlated errors**, ie. that some observed variables (answers to some questions) are pairwise correlated independently of a relationship with a factor being a common cause to all the analized questions.
   - Assuming that there is additional latent factor related to some of observed variables independently of a relationship with a factor being a common cause to all the analized questions.
   - Assuming that there are several subscales within our set of questions.

But how to determine, which (pairs/subsets of) questions are causing problems?
...
Because problem involves only two questions within CCFA approach it will be better to solve it adding to a model correlation between observed variables' errors.
...
We can see that such a model fits data much better than our base unidimensional model (however we can notice, that Q10 appears not to be conected with a second specific factor). It also fits data better than a model with two residual correlations added. However we can't directly test fit of this bi-factor model and a model with residual correlations (for example with `anova()` function), because they aren't nested.

However we should note that we encountered some **problems with estimation**. Because of that this results can't be fully trusted. Generaly we have to few observed variables to assure stable estimation of a model with 3 latent factors within CCFA approach. This won't be so serious problem within IRT approach:
